...
export default function AdminStoreShell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    fetch('/api/admin/me')
      ...
      .then((d) => {
        if (!d?.authenticated) {
          router.push('/admin/login?next=/admin/store')
        } else {
          setUsername(d.username)
        }
      })
      .catch(() => router.push('/admin/login?next=/admin/store'))
  }, [router])
