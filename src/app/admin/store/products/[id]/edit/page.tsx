...
interface ProductItem {
  ...
  tabs?: { label: string; content: string; order: number }[]
...
export default function EditProductPage() {
  ...
  useEffect(() => {
    ...
    fetch('/api/admin/products', { credentials: 'include' })
      .then(async (r) => {
        ...
        const product = (data.items || []).find((p: ProductItem) => p._id === id)
        if (!product) { setNotFound(true); return }
        // Populate form
        setTitle(product.title)
        setCategory(product.category)
        setPrice(String(product.price))
        setComparePrice(String(product.compareAtPrice || ''))
        setShortPitch(product.shortPitch || '')
        ...
        setDemoVideoLabel(product.demoVideoLabel || '')
        setTitleFontSize(product.titleFontSize || '')
        setHowToVideo(product.howToUseVideo || '')
        setRating(String(product.rating || 5))
        setActive(product.isActive)
        setDownloadUrl(product.downloadUrl || '')
        setButtonText(product.buttonText || '')
        setBuyButtonText(product.buyButtonText || '')
        setButtonColor(product.buttonColor || '')
        setCartButtonBg(product.cartButtonBg || '')
        setCartButtonTextColor(product.cartButtonTextColor || '')
        setCartButtonBorderColor(product.cartButtonBorderColor || '')
        setCartButtonHoverBg(product.cartButtonHoverBg || '')
        setCartButtonHoverTextColor(product.cartButtonHoverTextColor || '')
        setBuyButtonBg(product.buyButtonBg || '')
        setBuyButtonTextColor(product.buyButtonTextColor || '')
        setBuyButtonBorderColor(product.buyButtonBorderColor || '')
        setBuyButtonHoverBg(product.buyButtonHoverBg || '')
        setBuyButtonHoverTextColor(product.buyButtonHoverTextColor || '')
        setTabs(product.tabs || [])
        setTestimonials(product.testimonials || [])
        setFaqItems(product.faq || [])
        setTrustCards(product.trustCards || [])
        setSpecs(product.specs || [])
        setWhatsIncluded(product.whatsIncluded || [])
        setCoverageStats(product.coverageStats || [])
  ...
  async function handleSave(e: FormEvent) {
    ...
    try {
      ...
      setToast({ kind: 'success', text: 'Product updated!' })
  ...
  if (loading) {
    return <div className="empty"><span className="spinner" /> Loading product…</div>
  }

  if (notFound) {
    return (
      <div>
        <div className="admin-topbar"><div><h1>Product Not Found</h1></div></div>
        <div className="empty">
          <div className="icon">📦</div>
          <p>This product does not exist or has been deleted.</p>
          <Link href="/admin/store/products" className="btn btn-primary" style={{ marginTop: 16, display: 'inline-flex' }}>← Back to products</Link>
        </div>
      </div>
    )
  }
      ...
      <div style={{ background: '#0f172a', border: '1px solid #1e293b', borderRadius: 12, padding: 24 }}>
        <form onSubmit={handleSave} autoComplete="off">
          ...
          <div className="field">
            ...
            <textarea value={shortPitch} onChange={(e) => setShortPitch(e.target.value)} rows={2} />
          </div>
          ...
          <div className="field">
            ...
            {images.map((url, i) => (
              <div key={i} className="flex gap-2 items-center mb-2" style={{ background: 'rgba(14,165,233,0.04)', borderRadius: 8, padding: '4px 6px', border: dragOverIndex === i && dragIndex !== null && dragOverIndex !== dragIndex ? '1.5px dashed #0ea5e9' : '1.5px solid transparent' }}>
                <span className="flex-shrink-0 text-slate-500 select-none" style={{ fontSize: 16, cursor: 'grab' }}>⠿</span>
