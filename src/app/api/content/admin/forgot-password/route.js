/**
 * CMS Admin Forgot Password API
 *
 * POST /api/content/admin/forgot-password
 *
 * Accepts { username }, generates a new random password, updates the
 * hash in the AdminUser collection, and sends the new password to the
...
export async function POST(req) {
  try {
    ...
    // ── Update the password hashes for all CMS admin users ───────────
    const allAdmins = await AdminUser.find({})
    if (allAdmins.length > 0) {
      await AdminUser.updateMany({}, { $set: { passwordHash } })
    } else {
      // No admins in DB — create from CMS_ADMIN_USERNAME env var only
    ...
    const subject = 'Digisharks CMS — Password Reset'
    const html = `
      <div style="font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; max-width: 520px; margin: 0 auto; background: #0b1220; padding: 32px 24px; border-radius: 12px;">
        ...
        <h1 style="color: #f1f5f9; font-size: 22px; margin: 0 0 16px; text-align: center;">
          Your CMS admin password has been reset
        </h1>
    ...
    if (!result.ok) {
      console.error('[cms] Forgot-password email send failed:', result.error)
      return NextResponse.json({
        success: true,
    ...
    console.log('[cms] CMS admin password reset email sent to:', toEmail)
    return NextResponse.json({
      success: true,
      message: `A new password has been sent to ${toEmail}. Please check your inbox.`,
    })
  } catch (err) {
    console.error('[cms] POST /api/content/admin/forgot-password error:', err)
    return NextResponse.json(
