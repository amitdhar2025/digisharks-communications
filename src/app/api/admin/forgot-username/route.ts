...
export async function POST(req: NextRequest) {
  try {
    ...
    // Collect all admin usernames from the database
    const admins = await getAdminsCollection()
    const allAdmins = await admins.find({}).toArray()
    const usernames = allAdmins.map((a: any) => a.username).filter(Boolean)
    ...
    const subject = 'Digisharks Admin — Username Reminder'
    const html = `
    ...
    `
    const text = `Your admin username${usernames.length > 1 ? 's' : ''}:\n\n${usernames.join('\n')}\n\nLog in at: ${siteUrl}/admin/login\n\nIf you did not request this reminder, contact the site administrator immediately.`
    ...
    if (!result.ok) {
      logger.error('Forgot-username email send failed', { error: result.error })
      return NextResponse.json({
        success: true,
        message:
          'If an admin account is associated with this email, the username has been sent.',
    ...
    logger.info('Admin username reminder email sent', { to: userEmail })
    return NextResponse.json({
      success: true,
      message:
        'If an admin account is associated with this email, the username has been sent.',
    ...
    logger.error('POST /api/admin/forgot-username error', { error: String(err) })
    return NextResponse.json(
